...
2. 選択肢は4つ（正解は1つ）。
3. 解説(explanation)は、必ず各選択肢（1〜4）ごとに段落を分け、「選択肢1：【不適切】」のように**必ず番号と正誤判定を冒頭に明記**した上で詳細に解説すること。
4. 解説の末尾には、実務や試験対策に役立つ「ワンポイントアドバイス（Tips）」を積極的に追記すること。
5. ※重要：問題および解説は、最新の法令および税制（現在施行されている基準）に完全に準拠した内容にしてください。古い法令に基づいた出題は絶対に避けてください。
6. 出力は必ず以下のJSON形式のみとすること（マークダウン不要）。
